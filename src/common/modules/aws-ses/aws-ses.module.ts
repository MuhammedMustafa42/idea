...
import {
  awsSESDriverProvider,
  createAwsSESAsyncOptionProviders,
} from './providers';
...
export class AwsSESModule implements OnApplicationShutdown {
  ...
  static registerAsync(options: AwsSESModuleAsyncOptions): DynamicModule {
    if (!options.useFactory)
      throw new Error(
        'Missing Configurations for AwsSESModule: useFactory is required',
      );

    const providers: Provider[] = [
      createAwsSESAsyncOptionProviders(options),
      awsSESDriverProvider,
      AwsSESService,
      AppConfig,
    ];
